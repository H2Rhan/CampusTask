...
演进方向：AI 解析可平滑切换任意 OpenAI 兼容大模型服务（云端 API 或自建推理），
部署支持任意 Linux 云主机；客户端以微信小程序为先导，架构上与 App 形态解耦。
...
复杂度：O(k² · Dijkstra)，k ≤ 10 的校园场景下毫秒级；多任务批量分配已实现匈牙利算法
（`assignment_service.py`），进一步支持"一趟多单"可扩展为带容量约束的车辆路径问题（CVRP）。
