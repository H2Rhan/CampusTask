...
| 批量调度 | 匈牙利算法：全校任务总边际成本最小化分配 | ✅ |
| 运营看板 | 注册/接单率/完成率/均时/复购率/仲裁率 实时统计 | ✅ |
| 微信登录 | code2session 一键登录（dev 通道已通，待配置真实 AppID） | ✅ |
| 消息通知 | 订阅消息（取件/送达/验收提醒） | 🗓 规划中 |
...
- 多任务场景：`GET /match/batch-assign` 用匈牙利算法求**全校任务总边际成本最小**的指派方案（已实现）
...
- **v0.1（当前）**：任务撮合全流程闭环——AI 发布、边际成本匹配、状态机、钱包、聊天、仲裁、信用、运营看板
- **v0.2**：校园地图实地测绘校准（60+ 地点）、微信一键登录、订阅消息通知
- **v0.3**：津南校区内测上线、多任务顺路打包（一趟多单）、数据看板可视化
- **v1.0**：真实支付分账、多校区支持、位置实时共享

详见 [docs/ROADMAP.md](docs/ROADMAP.md)。

## ⚠️ 合规与安全说明

- 当前版本使用**虚拟钱包**；正式上线不自建资金池，接入微信支付等持牌机构
- 密码 PBKDF2 哈希存储，JWT 鉴权，接口级角色权限校验
- 聊天室仅任务双方可见，任务结束自动归档；位置数据最小化采集
- 校园地图 v0.1 为示意坐标，正式上线前实地测绘校准（见 ROADMAP）
